...
from utils.camutils import cam_to_label, get_valid_cam, multi_scale_cam2,multi_scale_cam3
...
parser.add_argument("--model_path", default="/home/newdisk/fty/LZ/MSFC/results/MSFC_vit-b_voc_20k.pth", type=str, help="model_path")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    cam_logit_dir = os.path.join(base_dir, "logits", args.infer_set)

    os.makedirs(cam_aux_dir, exist_ok=True)
    os.makedirs(cam_dir, exist_ok=True)
    os.makedirs(cam_logit_dir, exist_ok=True)
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            _cams, _cams_aux,fmp = multi_scale_cam3(model, inputs, [1.0, 0.5, 1.5])
