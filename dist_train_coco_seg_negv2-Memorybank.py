...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        #         m=0.9  #动量因子 m
        #         ClassMemoryBank[index,:]=ClassMemoryBank[index,:]*m+concatenated_tensor.squeeze().square()*(1-m)
        
        
        
        #cosine_similarity  = F.cosine_similarity(fmap, ClassMemoryBankBefore[..., None, None], dim=1).unsqueeze(0)  # class*h*w  没有考虑正则化，后期可以考虑# 计算特征图与类别memory bank之间的余弦相似度
        #_, pseudo_label_aux2 = cam_to_label(cosine_similarity.detach(), cls_label=cls_label, img_box=img_box, ignore_mid=True, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index) # 根据余弦相似性计算伪标签
        #aff_mask2 = label_to_aff_mask(pseudo_label_aux2)# 将伪标签转换为相似性掩码
        #MemoryPTC_loss = get_masked_ptc_loss(fmap, aff_mask2)# 计算基于相似性掩码的损失
        ...
        # loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss +args.w_seg * seg_loss + args.w_reg * reg_loss + 0.2*target_error_loss 
        # loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss +args.w_seg * seg_loss + args.w_reg * reg_loss + 0.2*Multiloss
        # loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss +args.w_seg * seg_loss + args.w_reg * reg_loss + 0.5*MemoryPTC_loss
        # loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss +0.1*target_error_loss +args.w_ptc*MemoryPTC_loss

        # loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss +args.w_seg * seg_loss + args.w_reg * reg_loss + 0.1*Multiloss + 0.5*MemoryPTC_loss
