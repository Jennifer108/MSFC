...
class PAR(nn.Module):
    ...
    def __init__(self, dilations, num_iter,devicePAR):
        super().__init__()
    ...
    def get_dilated_neighbors(self, x):
        ...
        for d in self.dilations:
            ...
            _x = F.conv2d(_x_pad.to(device=self.device, dtype=torch.float), self.kernel.to(device=self.device, dtype=torch.float), dilation=d).view(b, c, -1, h, w) 
            x_aff.append(_x) 
